...
describe PiecesController do
  ...
  describe "GET new" do
    it "should let me setup a new piece" do
      get :new, :portfolio_id => @user.login
  ...
  describe "POST create" do
    it "should allow me to create a piece with a price" do
      post :create, :portfolio_id => @user.login, :piece => { :title => Faker::Lorem.words(2).to_s, :for_sale => true, :price => 25.55 }
      response.should redirect_to(edit_user_portfolio_path(assigns['user'], assigns['portfolio']))
    ...
    it "should allow me to create a piece without a price" do
      post :create, :portfolio_id => @user.login, :piece => { :title => Faker::Lorem.words(2).to_s }
      response.should redirect_to(edit_user_portfolio_path(assigns['user'], assigns['portfolio']))
    ...
    it "should not let me create a piece if I give bogus info" do
      post :create, :portfolio_id => @user.login, :piece => { }
      response.should redirect_to(edit_user_portfolio_path(assigns['user'], assigns['portfolio']))
  ...
  describe "GET edit" do
    it "should allow me to edit a piece" do
      ...
      get :edit, :portfolio_id => @user.login, :id => @piece.id
      assigns['user'].should_not be_nil
      assigns['portfolio'].should_not be_nil
      assigns['service_types'].should_not be_nil
      assigns['page_title'].should_not be_nil
  ...
  describe "PUT update" do
    it "should let me update my piece" do
      @piece = Factory(:piece, :portfolio_id => @user.portfolio.id)
      put :update, :portfolio_id => @user.login, :id => @piece.id, :piece => { :title => Faker::Lorem.words(2) }
  ...
  describe "GET show" do
    it "should show me a piece I have uploaded" do
      @piece = Factory(:piece, :portfolio_id => @user.portfolio.id)
      get :show, :portfolio_id => @user.login, :id => @piece.id
